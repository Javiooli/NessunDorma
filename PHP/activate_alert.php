...
if (isset($_POST['userId']) && isset($_POST['currency'])) {
    ...
    if ($count > 0) {
        $updateStmt = $conn->prepare("UPDATE user_alerts SET active = 1, pending = 0 WHERE userId = ? AND currency = ?");
        $updateStmt->bind_param("is", $userId, $currency);
        $updateStmt->execute();
        $updateStmt->close();
        exit;
    } else {
        $stmt = $conn->prepare("INSERT INTO user_alerts (userId, currency, active, pending) VALUES (?, ?, 1, 0)");
